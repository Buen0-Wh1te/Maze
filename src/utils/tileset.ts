...
function readBitmaskFromBitmap(
  imageData: ImageData,
  tileX: number,
  tileY: number
): number {
  ...
  const topRightPixel = isPixelColored(
    imageData,
    startX + TILE_SIZE - 4,
    startY + 4
  );
  const centerRightPixel = isPixelColored(
    imageData,
    startX + TILE_SIZE - 4,
    centerY
  );
  const bottomRightPixel = isPixelColored(
    imageData,
    startX + TILE_SIZE - 4,
    startY + TILE_SIZE - 4
  );
  const bottomCenterPixel = isPixelColored(
    imageData,
    centerX,
    startY + TILE_SIZE - 4
  );
  const bottomLeftPixel = isPixelColored(
    imageData,
    startX + 4,
    startY + TILE_SIZE - 4
  );
...
async function findMatchingTile(
